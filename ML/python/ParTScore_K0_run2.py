...
class ParTScoreProducer(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch("RotateVtx_trackdphi", "F", lenVar="nRotateVtx")
    ...
    def analyze(self, event):
        ...
        for ivtx in range(len(SDVSecVtx)):
            ...
            vtx = SDVSecVtx[ivtx]
            if not ( (vtx.charge==0) and (vtx.nTracks==2) and (vtx.tracksSize==2) and (vtx.mass>0.44) and (vtx.mass<0.56) ):
                continue
            ...
            for itk in range(len(VtxRefitTrack_pt)):
                ...
                tk_p4.SetPhi(VtxRefitTrack_phi_rot[itk])
                tk_p4.SetM(mass)
                vtx_p4 += tk_p4
            ...
            dy = vtx.y - pvy
            dz = vtx.z - pvz
            pdotv = (dx * vtx_p4.Px() + dy * vtx_p4.Py() + dz * vtx_p4.Pz()) / math.sqrt(vtx_p4.P2()) / math.sqrt(dx*dx + dy*dy + dz*dz)
            vtx_pAngle = math.acos(pdotv)
            vtx_pt = vtx_p4.pt()
            vtx_mass = vtx_p4.mass()
            vtx_Lxy = vtx.Lxy

            new_branches["nRotateVtx"] += 1
            new_branches["RotateVtx_mass"].append(vtx_mass)
            new_branches["RotateVtx_mass_before"].append(vtx.mass)
            new_branches["RotateVtx_pt"].append(vtx_pt)
            new_branches["RotateVtx_pAngle"].append(vtx_pAngle)
            new_branches["RotateVtx_trackdphi"].append(self.deltaPhi(VtxTrack_phi_rot[0]-VtxTrack_phi_rot[1]))
