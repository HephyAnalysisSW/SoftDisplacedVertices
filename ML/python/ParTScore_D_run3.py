...
class ParTScoreProducer(Module):
    ...
    def ArraytoNumpy(self,array,dtype=np.float64):
        arr_np = []
        for i in array:
          arr_np.append(i)
        arr_np = np.array(arr_np,dtype=dtype)
        return arr_np
    ...
    def analyze(self, event):
        ...
        Jet_jetId = np.array([ord(x) for x in event.Jet_jetId], dtype=np.uint8)
        ...
        Jet_chHEF = self.ArraytoNumpy(event.Jet_chHEF)
        Jet_neHEF = self.ArraytoNumpy(event.Jet_neHEF)
        Jet_muonIdx1 = self.ArraytoNumpy(event.Jet_muonIdx1)
        Jet_muonIdx2 = self.ArraytoNumpy(event.Jet_muonIdx2)
        #Jet_nConstituents = self.ArraytoNumpy(event.Jet_nConstituents)
