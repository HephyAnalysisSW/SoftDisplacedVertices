...
class GNNGenInfo: public edm::one::EDProducer<edm::one::SharedResources> {
  public:
    explicit GNNGenInfo(const edm::ParameterSet &);
...
GNNGenInfo::GNNGenInfo(const edm::ParameterSet &iConfig)
...
std::unique_ptr<NNArray> GNNGenInfo::initializeNNs(const edm::ParameterSet &iConfig) {
...
void GNNGenInfo::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  initEventStructure();
  ...
  if (tracks->size() != tracks_isoDR03->size())
    throw cms::Exception("GNNGenInfo") << "Tracks mismatch with track IsoDR03!";
  ...
  for (auto& illp : llp_idx) {
    ...
    std::vector<int> llp_daus_all = SoftDV::GetDaughters(illp, genParticles, false);
    std::vector<int> llp_daus;
    for (int igen:llp_daus_all) {
      const reco::GenParticle& idau = genParticles->at(igen);
      if ((idau.charge()==0) || (fabs(idau.charge()<1)))
        continue;
      if (fabs(idau.eta())>2.5)
        continue;
      if (idau.pt()<0.5)
        continue;
      double r = std::hypot(idau.vx(),idau.vy());
      if (r>15)
        continue;
      llp_daus.push_back(igen);
    }
  ...
  if (llp_track_idx.size()!=2){
    throw cms::Exception("GNNGenInfo") << "llp_track_idx.size incorrect!";
  }
    ...
    for (int iist=0; iist<nst; ++iist){
      for (int jjst=iist+1; jjst<nst; ++jjst){
        ...
        if (ttk_ref_map.find(ist_ref)==ttk_ref_map.end())
          throw cms::Exception("GNNGenInfo") << "Track index " << ist << " not found in map!";
        if (ttk_ref_map.find(jst_ref)==ttk_ref_map.end())
          throw cms::Exception("GNNGenInfo") << "Track index " << jst << " not found in map!";
...
float GNNGenInfo::edge_dist(std::vector<float> v1, std::vector<float> v2) {
  if (v1.size() != v2.size()){
    throw cms::Exception("GNNGenInfo::edge_dist") << "Sizes of v1 and v2 do not match!";
  }
  float d2 = 0;
...
std::vector<float> GNNGenInfo::track_input(reco::TrackRef tk, const reco::Vertex* pv, SoftDV::PFIsolation isoDR03) {
  float pt = tk->pt();
  float eta = tk->eta();
  float phi = tk->phi();
...
bool GNNGenInfo::v_pass(const reco::Vertex& v, const reco::Vertex* pv) {
  if (!v.isValid())
    return false;
  if (v.normalizedChi2()>5)
    return false;
  double dx = (v.x() - pv->x());
  double dy = (v.y() - pv->y());
  double dz = (v.z() - pv->z());
...
bool GNNGenInfo::isGoodTrack(reco::TrackRef tk, const reco::Vertex* pv, SoftDV::PFIsolation isoDR03) {
  float pt = tk->pt();
  float eta = tk->eta();
  float phi = tk->phi();
  float dxy = tk->dxy(pv->position());
