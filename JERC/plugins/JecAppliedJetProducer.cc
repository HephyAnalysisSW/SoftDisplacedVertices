...
#include "SoftDisplacedVertices/JERC/interface/JecConfigReader.h"
#include "SoftDisplacedVertices/JERC/interface/JecApplication.h"
#include "SoftDisplacedVertices/JERC/interface/Year.h"
...
void JecAppliedJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  if (iEvent.isRealData()!=isData_)
    throw cms::Exception("JecAppliedJetProducer")
      << "The isData_ tag is incorrect!";
  ...
  JecApplication::Applier applier =
    [&]() -> JecApplication::Applier {
      if (isData_) {
        ...
        auto data_era = SoftDV::getEra(iEvent.id().run());
        if (data_era.first!=year_){
          throw cms::Exception("JecAppliedJetProducer") << "Data year " << data_era.first << " does not match with the given "<< year_ << "!";
        }
        year_ = data_era.first;
        std::string era_;                // required for data, optional/ignored for MC
        era_ = data_era.second;
