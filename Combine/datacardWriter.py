...
# Example usage:
# python3 datacardWriter.py all_sig_2018 all_bkg_2018 testK/histograms/MC_SP SP_evt.MET_pt_vs_SPMaxLxySig testK/datacards/
...
def write_workspace_file(fileOut, count_dict, error_dict):
    ...
    for region in DC.bins:
        ### ----------- Care here!! ----------- ###
        ###  predictions are inserted instead of observations  ###
        if region == "A":
            try:
...
@click.command()
@click.argument('sig',         default=None)
@click.argument('bkg',         default=None)
@click.argument('histdir',     default=None)
@click.argument('histname',    default=None)
@click.argument('datacarddir', default=None)
@click.argument('debug',       default=False)
def main_cli(sig, bkg, histdir, histname, datacarddir, debug):
    for MET_bound in range(400, 1000+1, 50):
        for LxySig_bound in range(15, 100+1, 5):
            ...
            bkg_onepluserror_dict = {}
            for region, squared_error in bkg_squared_error_dict.items():
                if bkg_count_dict[region] != 0:
                    bkg_onepluserror_dict[region] = 1+ math.sqrt(squared_error)/bkg_count_dict[region]
                else:
                    bkg_onepluserror_dict[region] = 1
            ...
            for sample in sampleDict['sig']:
                ...
                sig_onepluserror_dict = {}
                for region, squared_error in sig_squared_error_dict.items():
                    if sig_count_dict[region] != 0:
                        # K = 1+Delta(x)/x
                        sig_onepluserror_dict[region] = 1+ math.sqrt(squared_error)/sig_count_dict[region]
                    else:
                        sig_onepluserror_dict[region] = 1

                count_dict['sig'] = sig_count_dict
                error_dict['sig'] = sig_onepluserror_dict
            
                sigDir = os.path.join(datacarddir, sample.name)
                os.makedirs(datacarddir, exist_ok=True)
                os.makedirs(sigDir,exist_ok=True)
                outTag = os.path.join(sigDir, f"MC_{sample.name}_{xSplit_val:.3g}_{ySplit_val:.3g}")
