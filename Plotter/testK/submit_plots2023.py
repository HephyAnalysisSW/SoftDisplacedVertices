...
config =           "$CMSSW_BASE/src/SoftDisplacedVertices/Plotter/configs/2023_calc_yields.yaml"
outDir_base = "/scratch-cbe/users/alikaan.gueven/AN_plots/"
work_subdir = "checks_2023"
unique_dir  = "val_yields"
...
for s_type in samples_to_plot.keys():
    ...
    for sample in samples_to_plot[s_type]:
        if s_type != 'data':
            command = f'submit_to_cpu_medium.sh "python3 -u {autoplotter_path}  --sample {sample} --output {outDir} --config {config} --lumi 19600 --json {json_db[s_type]} --datalabel {tier[s_type]} --year {year}"'
        else:
