...
The input is a 2D histogram with x = m(pi pi) and y = Lxy. In each
visible Lxy bin, the script projects the mass distribution, fits the two K0S
sidebands with a first-order polynomial, integrates the fitted background under
the K0S signal window, and subtracts it from the observed signal-window yield.
...
deviations for Lxy >= 1 cm. The final visible bin includes all entries above
20 cm as a separate overflow-style plotted bin, including the ROOT overflow bin.
...
samples = {
    "2022_pre": {
        ...
        "outdir": f"{base}/plots/k0_vtx_reco_unc_2022_pre",
    ...
    "2022_post": {
        ...
        "outdir": f"{base}/plots/k0_vtx_reco_unc_2022_post",
    ...
    "2023_pre": {
        ...
        "outdir": f"{base}/plots/k0_vtx_reco_unc_2023_pre",
    ...
    "2023_post": {
        ...
        "outdir": f"{base}/plots/k0_vtx_reco_unc_2023_post",
...
def run_sample(sample_name, sample):
    ...
    # Extract the background-subtracted K0 yield separately in each visible Lxy bin.
    for ibin in visible_lxy_bins:
        ...
        lxy_low = lxy_axis.GetBinLowEdge(ibin)
        lxy_high = lxy_axis.GetBinUpEdge(ibin)
    ...
    data_count, data_background, data_yield, data_error = yield_in_lxy_bin(
        h_data, first_overflow_lxy_bin, last_overflow_lxy_bin
    )
    mc_count, mc_background, mc_yield, mc_error = yield_in_lxy_bin(
        h_mc, first_overflow_lxy_bin, last_overflow_lxy_bin
    )
    lxy_low = lxy_axis.GetBinLowEdge(first_overflow_lxy_bin)
    lxy_high = lxy_axis.GetBinUpEdge(first_overflow_lxy_bin)

    rows.append(
        {
            "bin": first_overflow_lxy_bin,
            "lxy_low": lxy_low,
            "lxy_high": lxy_high,
            "lxy": 0.5 * (lxy_low + lxy_high),
            "data_signal_count": data_count,
    ...
    bin_edges = [r["lxy_low"] for r in rows] + [rows[-1]["lxy_high"]]
