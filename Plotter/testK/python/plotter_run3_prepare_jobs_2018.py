...
samples_to_plot = {
    'sig' : [sample.name for sample in ss.sig_ABCD_study],
    'bkg' : [sample.name for sample in ss.all_bkg_2018],
    'data': [sample.name for sample in ss.met_2018]
}

json_db = {
    'sig'  : 'clip_IVF_ABCD_study.json',
    'bkg'  : 'clip_IVF_ABCD_study.json',
    'data' : 'clip_IVF_data.json'
}

year = 2018
autoplotter_path = "$CMSSW_BASE/src/SoftDisplacedVertices/Plotter/autoplotter.py"
config =           "$CMSSW_BASE/src/SoftDisplacedVertices/Plotter/configs/plotconfig_Run2_MLscore_first.yaml"
outDir_base = "/scratch-cbe/users/alikaan.gueven/AN_plots/"
work_subdir = "ParT_hists"
unique_dir  = "plotconfig_Run2_MLscore_first"
files_per_job = 2
