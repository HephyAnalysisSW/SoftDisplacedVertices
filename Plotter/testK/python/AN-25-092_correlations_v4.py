...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-AN-25-092-correlations-v4")
for thread_var in (
    "OMP_NUM_THREADS",
...
def make_tk_table(events):
    ...
    sv_phi = ak.broadcast_arrays(
        arrays["SDVSecVtx_L_phi"][..., np.newaxis],
        arrays["SDVTrack_pt"],
    )[0]
...
def make_sv_table(events):
    return to_flat_dataframe(
        {
            "sv_log_pt": np.log(events["SDVSecVtx_pt"]),
            "sv_L_eta": events["SDVSecVtx_L_eta"],
            "sv_log_LxySig": np.log(events["SDVSecVtx_LxySig"]),
            "sv_pAngle": events["SDVSecVtx_pAngle"],
            "sv_charge": events["SDVSecVtx_charge"],
            "sv_chi2_over_ndof": events["SDVSecVtx_chi2"] / events["SDVSecVtx_ndof"],
            "sv_sum_tkW_over_tracksSize": events["SDVSecVtx_sum_tkW"] / events["SDVSecVtx_tracksSize"],
            "sv_closestJetdR": closest_jet_dr(events),
            "vtx_PART_1111best_valloss_epoch": events["vtx_PART_1111best_valloss_epoch"],
            "sv_matchedLLPnDau_bydau": events["SDVSecVtx_matchedLLPnDau_bydau"],
        },
        SV_COLUMNS,
    )
...
def save_outputs(level, corr):
    out_csv = OUT_DIR / f"AN-25-092_correlations_v4_{level}.csv"
    out_png = OUT_DIR / f"AN-25-092_correlations_v4_{level}.png"
    corr.to_csv(out_csv)
    plot_corr(corr, out_png)
