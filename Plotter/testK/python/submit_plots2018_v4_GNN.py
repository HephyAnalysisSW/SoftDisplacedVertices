...
samples_to_plot = {
    'sig' : [sample.name for sample in ss.sig_test_2018],
    'bkg' : [sample.name for sample in ss.all_bkg_2018],
    # 'data': [sample.name for sample in ss.met_2018]
}

json_db = {
    'sig'  : 'clip_GNN_testK.json',
    'bkg'  : 'clip_GNN_testK.json',
...
config =           "$CMSSW_BASE/src/SoftDisplacedVertices/Plotter/configs/2018/vtx_PART_1112best_valloss_epoch.yaml"
outDir_base = "/scratch-cbe/users/alikaan.gueven/AN_plots/"
work_subdir = "ParT_hists"
unique_dir  = "vtx_PART_1112best_valloss_epoch"
files_per_job = 2
...
for s_type in samples_to_plot.keys():
    ...
    for sample in samples_to_plot[s_type]:
        ...
        # ------------------------------------------------------------
            if s_type != 'data':
                command = f'sh/submit_to_cpu_rapid.sh "python3 -u {autoplotter_path}  --sample {sample} --filelist {fileList_path} --postfix {i} --output {outDir} --config {config_copy} --lumi 59683 --json {json_db[s_type]} --datalabel {tier[s_type]} --year {year}"'
            else:
                command = f'sh/submit_to_cpu_rapid.sh "python3 -u {autoplotter_path}  --sample {sample} --filelist {fileList_path} --postfix {i} --output {outDir} --config {config_copy} --lumi -1 --json {json_db[s_type]} --datalabel {tier[s_type]} --year {year} --data"'
