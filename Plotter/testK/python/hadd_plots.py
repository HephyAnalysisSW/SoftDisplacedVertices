...
dirs = {
    ...
    "run2": os.path.join(work_dir, "hadd_plots_run2"),
}
...
def build_signal_groups(sig_dir, year):
    glob_patterns = []
    glob_patterns.append(os.path.join(sig_dir, f"stop_M*_ct*_{year}_hist*.root"))
    glob_patterns.append(os.path.join(sig_dir, f"stopML_M*_ct*_{year}_hist*.root"))
    glob_patterns.append(os.path.join(sig_dir, f"stopMLstudy_M*_ct*_{year}_hist*.root"))
    glob_patterns.append(os.path.join(sig_dir, f"C1N2MLstudy_M*_ct*_{year}_hist*.root"))
    glob_patterns.append(os.path.join(sig_dir, f"C1N2ML_M*_ct*_{year}_hist*.root"))
    ...
    re_patterns = []
    re_patterns.append(re.compile(rf"(stop_M\d+_\d+_ct[^_]+_{year})_hist\d+\.root$"))
    re_patterns.append(re.compile(rf"(stopML_M\d+_\d+_ct[^_]+_{year})_hist\d+\.root$"))
    re_patterns.append(re.compile(rf"(stopMLstudy_M\d+_\d+_ct[^_]+_{year})_hist\d+\.root$"))
    re_patterns.append(re.compile(rf"(C1N2MLstudy_M\d+_\d+_ct[^_]+_{year})_hist\d+\.root$"))
    re_patterns.append(re.compile(rf"(C1N2ML_M\d+_\d+_ct[^_]+_{year})_hist\d+\.root$"))
...
def hadd_year(year):
    ...
    if os.path.isdir(sig_dir):
        sig_groups = build_signal_groups(sig_dir, year)
...
def build_run2_signal_groups():
    ...
    for year in YEARS:
        ...
        year_files = []
        for pattern in [
            os.path.join(sig_dir, f"stop_M*_ct*_{year}_hist.root"),
            os.path.join(sig_dir, f"stopML_M*_ct*_{year}_hist.root"),
            os.path.join(sig_dir, f"stopMLstudy_M*_ct*_{year}_hist.root"),
            os.path.join(sig_dir, f"C1N2MLstudy_M*_ct*_{year}_hist.root"),
            os.path.join(sig_dir, f"C1N2ML_M*_ct*_{year}_hist.root"),
        ]:
