...
for s_type in samples_to_plot.keys():
    ...
    for sample in samples_to_plot[s_type]:
        ...
        # ------------------------------------------------------------
            if s_type != 'data':
                command = f'submit_to_cpu_short.sh "python3 -u {autoplotter_path}  --sample {sample} --filelist {fileList_path} --postfix {i} --output {outDir} --config {config} --lumi 7980.4 --json {json_db[s_type]} --datalabel {tier[s_type]} --year {year}"'
            else:
