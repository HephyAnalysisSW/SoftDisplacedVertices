...
samples_to_plot = {
    ...
    # 'bkg' : [sample.name for sample in ss.all_bkg_2022],
    # 'data': [sample.name for sample in ss.met_2022]
}

tier = {'sig'  : 'CustomNanoAODv3',
        'bkg'  : 'CustomNanoAOD',
        'data' : 'CustomNanoAOD'
}

json_db = {'sig'  : 'PrivateSignal_v3.json',
...
config =           "$CMSSW_BASE/src/SoftDisplacedVertices/Plotter/configs/2022_checks/2022_check_2.yaml"
...
for s_type in samples_to_plot.keys():
    ...
    for sample in samples_to_plot[s_type]:
        if s_type != 'data':
            command = f'{testKDir}/submit_to_cpu_short.sh "python3 -u {autoplotter_path}  --sample {sample} --output {outDir} --config {config} --lumi 7980.4 --json {json_db[s_type]} --datalabel {tier[s_type]} --year {year}"'
        else:
            command = f'{testKDir}/submit_to_cpu_short.sh "python3 -u {autoplotter_path}  --sample {sample} --output {outDir} --config {config} --lumi -1 --json {json_db[s_type]} --datalabel {tier[s_type]} --year {year} --data"'
    ...
    out_json_path = os.path.join(outDir, 'job_ids2022.json')
