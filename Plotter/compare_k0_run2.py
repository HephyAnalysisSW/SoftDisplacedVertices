...
parser.add_argument('--bkg17', type=str, nargs='+',default=[],
                    help='background files to compare')
parser.add_argument('--bkg18', type=str, nargs='+',default=[],
                    help='background files to compare')
...
parser.add_argument('--bkg_scale17', type=float, default=1,
                    help='Scale factors for bkg MC')
parser.add_argument('--bkg_scale18', type=float, default=1,
                    help='Scale factors for bkg MC')
...
def comparehists_cms(name,hs,colors,legends,sig_scale=[], scale_to_data=False, ratio=True, norm=False):
  ...
  if not args.bkg_scale==1:
    if bkg_mc is not None and bkg_mc.Integral(0,1000000)!=0:
      bkg_mc.Scale(args.bkg_scale)
      for h in d_bkg:
        d_bkg[h].Scale(args.bkg_scale)
  ...
  if ratio:
    ...
    if name=="SDVSecVtx_Lxy":
      print("{} Data/MC ratio is {}".format(name,np.array2string(np_ratio, separator=', ')))
      print("{} Data/MC ratio Yerror high is {}".format(name,np.array2string(np_yerr_high, separator=', ')))
      print("{} Data/MC ratio Yerror low is {}".format(name,np.array2string(np_yerr_low, separator=', ')))
      print("------------------------------------------------------")
...
def makeplots(datafn, bkgfns, sigfns,bkglegend,siglegend,bkgcolors,sigcolors,sig_scale,scale_to_data,ratio,norm):
  ...
  fs = {
      ...
      'bkg17': [ROOT.TFile.Open(fn) for fn in bkg17fns],
      'bkg18': [ROOT.TFile.Open(fn) for fn in bkg18fns],
  ...
  if (args.dirs is None) or (len(args.dirs)==0):
    ...
    if len(fs['data'])>0:
      ...
      fdir = fs['bkg17'][0].Get(dirs)
    elif len(fs['bkg18'])>0:
      fdir = fs['bkg18'][0].Get(dirs)
    else:
