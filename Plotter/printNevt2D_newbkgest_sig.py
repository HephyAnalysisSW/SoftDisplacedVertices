...
def getEvtsHEM(fn,SRnames):
    ...
    for SRname in SRnames:
      ...
      nevt_sig_uncert = ctypes.c_double(0)
      fs_sig.append(ROOT.TFile.Open(os.path.join(args.inputHEM,fn)))
      hsig = fs_sig[-1].Get(SRname)
      binlow = hsig.GetXaxis().FindBin(args.metcut)
      xcut = hsig.GetXaxis().FindBin(700)
      #xcut = hsig.GetXaxis().FindBin(375)
...
def getEvts_total(fn,SRnames,HEM):
  if not HEM:
    return getEvts(fn,SRnames)
  fHEM = 0.6477
  evt = getEvts(fn,SRnames)
  evtHEM = getEvtsHEM(fn,SRnames)
  out_d = {}
  nevt_sigs = {}
  nevt_uncert_sigs = {}
  for r in evt.keys():
    ...
    for rr in ['A','B','C','D']:
      nevt = evt[r][rr].n
      nevtHEM = evtHEM[r][rr].n
      if nevt==0:
        f = 1
      else:
        f = (nevt*(1-fHEM)+nevtHEM*fHEM)/nevt
      nevt_final = evt[r][rr]*f
      out_d[r][rr] = nevt_final
...
if __name__=="__main__":
  ...
  HEM = False
  if args.inputHEM is not None:
    HEM = True

  for i in range(len(sig_fns)):
    #devts = getEvts(sig_fns[i],args.SR)
    devts = getEvts_total(sig_fns[i],args.SR,HEM)
    year = ''
