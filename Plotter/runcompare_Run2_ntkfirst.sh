...
for m in 200 250 300 350
do
    for s in 'GT0' 'GT1' 'GT2' 'GTgt3'
    do
      for o in '_SDVSecVtx_leading' '_SDVSecVtx_leading_ML0p2' '_SDVSecVtx_leading_ML0p5' '_SDVSecVtx_leading_ML0p8' '_SDVSecVtx_leading_ML0p9' '_SDVSecVtx_leading_ML0p95' '_SDVSecVtx_leading_ML0p99' '_SDVSecVtx_leading_ML0p995' '_SDVSecVtx_leading_ML0p999' '_SDVSecVtx_leading_ML1'
      do
        python3 compare_data_new.py  --bkg ${1}/st_2018_hist.root ${1}/ttbar_2018_hist.root ${1}/qcd_2018_hist.root ${1}/wjets_2018_hist.root ${1}/zjets_2018_hist.root  --bkgnice "Single top" "TTbar" "QCD" "WJets" "ZJets" --signal ${1}/C1N2ML_M200_195_ct20_2018_hist.root ${1}/C1N2ML_M200_185_ct20_2018_hist.root --signice "dm=5GeV" "dm=15GeV" --output /groups/hephy/cms/ang.li/SDV/SigBkgComp_2018_run23_ngtfirst_${2}_MET$m$s$o --dirs "MET$m$s$o" --commands "h.Rebin(5) if ('LxySig' in h.GetName()) else h.GetXaxis().SetRangeUser(0,20) if ('pfRel' in  h.GetName()) else None" --norm
      done
    done
done
