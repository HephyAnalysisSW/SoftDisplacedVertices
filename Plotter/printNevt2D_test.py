...
if __name__=="__main__":
  ...
  for i in range(len(sig_fns)):
    ...
    #if 'SR' in devts:
    #  print("\\multirow{{3}}{{*}}{{{}}} & Tight signal plane & ${:.02fL}$ &  ${:.02fL}$ & ${:.02fL}$ & ${:.02fL}$ \\\\".format(sig_fns[i].replace('_2017_hist.root','').replace('_2018_hist.root',''),devts.get('SR')['A'],devts.get('SR')['B'],devts.get('SR')['C'],devts.get('SR')['D']))
    #else:
    #  print("Tight signal plane ($\\ngoodtrack \\geq 2$) & -- &  -- & -- & -- \\\\")
    #print("&Prediction & ${:.02fL}$ &  ${:.02fL}$ & ${:.02fL}$ & ${:.02fL}$ \\\\".format(predict(devts,'SR','A'),predict(devts,'SR','B'),predict(devts,'SR','C'),predict(devts,'SR','D')))
    #print("&Loose signal plane     & ${:.02fL}$ &  ${:.02fL}$ & ${:.02fL}$ & ${:.02fL}$ \\\\".format(devts.get('VR1')['A'],devts.get('VR1')['B'],devts.get('VR1')['C'],devts.get('VR1')['D']))
    #print("&Prediction     & ${:.02fL}$ &  -- & ${:.02fL}$ & -- \\\\".format(predict(devts,'VR1','A'),predict(devts,'VR1','C')))
