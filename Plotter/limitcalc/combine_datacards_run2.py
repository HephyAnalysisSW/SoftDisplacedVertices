...
DATACARDS_DIR = "/scratch-cbe/users/alikaan.gueven/AN_plots/ParT_hists/plotconfig_Run2_goodtk_first_v2/datacards"
...
os.makedirs(os.path.join(DATACARDS_DIR, 'run2'), exist_ok=True)
for (prefix, suffix), cards in sorted(groups.items()):
    if "2017" not in cards or "2018" not in cards:
        continue

    output_path = os.path.join(DATACARDS_DIR, f"run2/{prefix}{suffix}")
    cmd = (
        f"combineCards.py "
